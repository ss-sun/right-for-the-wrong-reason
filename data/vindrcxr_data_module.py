...
class Vindr_CXR(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        '''
        data = {}
        img_path = os.path.join(self.image_dir, self.df.iloc[idx]['image_id']+'.png')
...
class Vindr_CXRDataModule(LightningDataModule):
    ...
    def setup(self):
        ...
        self.train_set = Vindr_CXR(image_dir=self.train_dir, df=self.train_df, train_diseases=self.TRAIN_DISEASES, transforms=self.data_transforms['train'])
        self.valid_set = Vindr_CXR(image_dir=self.valid_dir, df=self.valid_df, train_diseases=self.TRAIN_DISEASES, transforms=self.data_transforms['test'])
        self.test_set = Vindr_CXR(image_dir=self.test_dir, df=self.test_df, train_diseases=self.TRAIN_DISEASES, transforms=self.data_transforms['test'])
    ...
    def subset(self, src_dir, src_df, disease, label, transforms):
        ...
        subset = Vindr_CXR(image_dir=src_dir, df=filtered_df, train_diseases=self.TRAIN_DISEASES, transforms=transforms)
        return subset
...
if __name__ == '__main__':
    ...
    vindr_dict = {
        "dataset_dir": "/mnt/qb/work/baumgartner/sun22/contaimated_data_resize/vindrcxr/Aortic_enlargement/stripe/degree1.0",
    ...
    datamodule = Vindr_CXRDataModule(vindr_dict,
