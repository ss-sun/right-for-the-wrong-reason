...
#         # AdaIn
#         self.adain = AdaIn(out_channel)
#         # self.lrelu = nn.LeakyReLU()  # previous task switching model use leakyrelu
#         self.act_func = act_func
#         # Convolutional layers
#         self.conv1 = nn.Conv2d(in_channel, out_channel, kernel, stride=stride, padding=padding, bias=bias)
#
...
#     """Generator network."""
#
#     def __init__(self, num_classes, img_size, num_masks, act_func, n_fc=8, dim_latent=512, conv_dim=64, in_channels=1, repeat_num=6, use_tanh=True, deep_supervise=False):
#         super(Generator_with_Ada, self).__init__()
#
#         self.num_classes = num_classes
#         self.img_size = img_size
...
#
#         if act_func == "relu":
#             activation_func = nn.ReLU(inplace=True)  # previous starGAN model use relu
#         if act_func == "leakyrelu":
#             activation_func = nn.LeakyReLU(inplace=True)  # previous task switching model use leakyrelu
#
#         self.fcs = Intermediate_Generator(n_fc, dim_latent)
#
#
...
#             kernel_size = int(self.img_size / 4)
#             self.BN_c0 = nn.Conv2d(self.bn_out_channel, 1, kernel_size=kernel_size, bias=False)
#             self.BN_c1 = nn.Conv2d(self.bn_out_channel, 1, kernel_size=kernel_size, bias=False)
#             self.BN_c2 = nn.Conv2d(self.bn_out_channel, 1, kernel_size=kernel_size, bias=False)
#             self.BN_c3 = nn.Conv2d(self.bn_out_channel, 1, kernel_size=kernel_size, bias=False)
#             self.BN_c4 = nn.Conv2d(self.bn_out_channel, 1, kernel_size=kernel_size, bias=False)
#             self.BN_c5 = nn.Conv2d(self.bn_out_channel, 1, kernel_size=kernel_size, bias=False)
#             # self.n_ones = 20
#             # self.BN_classifiers = {}
#             # self.BN_classifiers = {}
#             # for i in range(self.num_disease):
#             #     kernel_size = int(self.img_size / 4)
#             #     BN_c = nn.Conv2d(self.bn_out_channel, 1, kernel_size=kernel_size, bias=False)
#             #     self.BN_classifiers[i] = BN_c
#             #self.register_parameter('deep_classifiers', self.BN_classifiers)
#             # print('BN_classifiers',self.BN_classifiers)
#
#
#
#     def forward(self, x, latent_z):
...
#
#         # print("inside model xmin", torch.min(x))
#         # print("inside model xmax", torch.max(x))
#
...
#
#         if self.deep_supervise:
#             for j in np.arange(0, self.dim_latent * self.num_classes, self.dim_latent):
#                 if latent_z[0][:, j] == 1:
#                     c = j/self.dim_latent
#                     if c == 0:
#                         BN_c = self.BN_c0
...
#         out = self.conv1(out)
#
#         ### original way to compose masks
#         y = x + out
#         # y = torch.tanh(y) if self.use_tanh else y
#         y = torch.tanh(y)
#         m = y - x
#
#         # m = torch.tanh(out) # m in range(-1, 1)
#         # y = x + m # x in range(-1, 1), therefore y in (-2, 2)
#         # y = y * 0.5 # scale y to (-1, 1)
#
#         if self.deep_supervise:
#             return y, m, BN_c(BN_out)
#         else:
#             return y, m
#
#
...
#
#         if act_func == "relu":
#             activation_func = nn.ReLU()  # previous starGAN model use relu
#         if act_func == "leakyrelu":
#             activation_func = nn.LeakyReLU()  # previous task switching model use leakyrelu
#
#         self.fcs = Intermediate_Generator(n_fc, dim_latent)
#         self.layer0 = convrelu(1, conv_dim, kernel=4, stride=1, padding=1, dim_latent=dim_latent, bias=False, act_func=activation_func)
#         curr_dim = conv_dim
...
# class CenterLoss(nn.Module):
#
#     # implementation get from https://github.com/KaiyangZhou/pytorch-center-loss/blob/master/center_loss.py
#     """Center loss.
#
#     Reference:
